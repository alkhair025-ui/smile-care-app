...
export default function BookingPortal() {
  ...
  useEffect(() => {
    const rawTenantId = Array.isArray(tenant_id) ? tenant_id[0] : tenant_id;
    if (!rawTenantId) { setLoading(false); setNotFound(true); return; }
    api.publicClinic(rawTenantId).then(setClinic).catch(() => setNotFound(true)).finally(() => setLoading(false));
  }, [tenant_id]);
    ...
    <View style={styles.root}>
      <LinearGradient colors={[colors.brand, colors.brandDark]} style={styles.header}>
        <SafeAreaView edges={['top']}>
          <Text style={styles.clinicName}>{clinic?.clinic_name || 'العيادة'}</Text>
