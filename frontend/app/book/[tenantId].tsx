...
export default function PatientPortal() {
  ...
  useEffect(() => {
    // Expo Router may return params as string or string[]
    const rawToken = Array.isArray(token) ? token[0] : token;
    if (!rawToken) { setLoading(false); setNotFound(true); return; }
    api.publicPatientPortal(rawToken)
      .then((d) => { console.log('portal loaded', d?.patient?.full_name); setData(d); })
      .catch((e) => { console.warn('portal error', e?.message); setNotFound(true); })
    ...
    <View style={styles.root}>
      <LinearGradient colors={[colors.brand, colors.brandDark]} style={styles.header}>
        <SafeAreaView edges={['top']}>
          <View style={styles.headerRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.clinicName}>{data.clinic?.name || 'العيادة'}</Text>
              <Text style={styles.clinicSub}>الملف الطبي للمريض</Text>
            </View>
          </View>
          <Text style={styles.patientName}>{data.patient?.full_name || 'المريض'}</Text>
        </SafeAreaView>
      </LinearGradient>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxxl }}>
        ...
        <View style={styles.finRow}>
          <FinTile label="إجمالي الفواتير" value={money(data.financials?.total_billed || 0)} color={colors.info} />
          <FinTile label="المدفوع" value={money(data.financials?.total_paid || 0)} color={colors.success} />
          <FinTile label="المتبقي" value={money(data.financials?.remaining || 0)} color={(data.financials?.remaining || 0) > 0 ? colors.error : colors.success} />
        </View>
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>{data.clinic?.name || 'العيادة'} · {data.clinic?.phone || ''}</Text>
